const convertUserInputToUTC=(data:{date:Date | string,period:'AM' | "PM",hour:number,minutes:number})=>{
    const selectedDate = new Date(data.date);
    const formattedHour = data.period === "PM" && data.hour !== 12 ? data.hour + 12 : data.hour === 12 && data.period === "AM" ? 0 : data.hour;
...
export default {convertUserInputToUTC,convertUserInputToSEUTC,convertLocalDateToUTC}