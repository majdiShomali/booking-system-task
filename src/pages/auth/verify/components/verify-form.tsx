...
import { EOtpType } from "@/types/otp.types";
    ...
    <Card className="w-full max-w-md mx-auto">
      <CardHeader>
        ...
        <CardDescription>
          ...
          {session?.user && 
          <span className="text-primary text-sm">{session?.user.email}</span>
          }
        ...
        >
          <div className="space-y-4 px-5">
            ...
            <Button
              type="submit"
              className="w-full"
              disabled={formData.otp.length !== 6 || isSubmitting || !session?.user?.verified}
            >
              {isSubmitting ? (
                <>
      ...
      <CardFooter className="flex flex-col items-center space-y-2">
        ...
        <ResendOtpTimer  time={35} handleResendOTP={handleResendOTP} disabled={false}/>
